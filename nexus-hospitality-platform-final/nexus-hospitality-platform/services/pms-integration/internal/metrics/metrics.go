// Package metrics provides tenant-aware Prometheus metrics for PMS integration.
package metrics
...
func NewCollector(cfg Config) *Collector {
	...
	c.registerCounter("http_requests_total", "Total HTTP requests", []string{"tenant_id", "method", "status_code"})
	c.registerHistogram("http_request_duration_seconds", "HTTP request duration", []string{"tenant_id", "method"})
	c.registerCounter("saga_started_total", "Total sagas started", []string{"tenant_id", "saga_type"})
	c.registerCounter("saga_completed_total", "Total sagas completed", []string{"tenant_id", "saga_type"})
	c.registerCounter("saga_failed_total", "Total sagas failed", []string{"tenant_id", "saga_type"})
	go c.serve()
	return c
}
...
func (c *Collector) IncrementCounter(name, tenantID, method, statusCode string) {
	if !c.enabled {
		return
	}
	c.mu.RLock()
	...
	vec.WithLabelValues(tenantID, method, statusCode).Inc()
}

// HistogramObserve records a histogram observation.
func (c *Collector) HistogramObserve(name string, value float64, tenantID, method string) {
	if !c.enabled {
		return
	}
	c.mu.RLock()
	...
	vec.WithLabelValues(tenantID, method).Observe(value)
}
