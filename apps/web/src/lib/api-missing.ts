...
export async function getCompetitorRates(hotelId: string): Promise<CompetitorRate[]> {
  const data = await api<{ rates: CompetitorRate[] }>(`/v1/competitors/${hotelId}/rates`);
  return data.rates || [];
}
...
export async function applyRateRecommendation(id: string): Promise<RateRecommendation> {
  return api(`/v1/rate-recommendations/${id}/apply`, { method: "POST" });
}
...
export async function createUpsellOffer(payload: Omit<UpsellOffer, "id" | "tenant_id" | "created_at" | "updated_at">): Promise<UpsellOffer> {
  return api("/v1/upsell-offers", { method: "POST", body: JSON.stringify(payload) });
}

export async function updateUpsellOffer(id: string, payload: Partial<UpsellOffer>): Promise<UpsellOffer> {
  return api(`/v1/upsell-offers/${id}`, { method: "PATCH", body: JSON.stringify(payload) });
}

export async function deleteUpsellOffer(id: string): Promise<void> {
  return api(`/v1/upsell-offers/${id}`, { method: "DELETE" });
}

/* ─── Villa Reservation Management ─── */
